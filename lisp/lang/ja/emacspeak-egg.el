...
;;; $Id: emacspeak-egg.el,v 1.2 2004/11/28 09:13:39 inoue Exp $
;;; $Author: inoue $ 
...
(eval-when-compile
  (require 'cl)
  (require 'dtk-speak)
  (require 'emacspeak-m17n "emacspeak-m17n")
)
...
(defadvice egg-convert-region (after emacspeak pre act)
  ...
  (emacspeak-egg-speak-after-conversion))
...
(defadvice its-buffer-ins/del-SYL (after   emacspeak pre act )
  ...
  (if (and ad-return-value
      ...
      (dtk-speak
       (emacspeak-ja-convert-string-to-cursor
	(copy-sequence (its-get-output newsyl))) nil 'ja))
  ad-return-value)

(defadvice its-backward-SYL (after emacspeak pre act)
  (emacspeak-speak-this-char (char-after (1+ (point))) 'ja)
)

(defadvice its-forward-SYL (after emacspeak pre act)
  (emacspeak-speak-this-char (char-after (1+ (point))) 'ja)
)

(defadvice its-delete-backward-SYL (before emacspeak pre act)
  "Speak deleted chars."
  (let* ((inhibit-read-only t)
	 (dtk-stop-immediately t)
	 (syl (get-text-property (1- (point)) 'its-syl))
	 (text (buffer-substring (- (point) (length (its-get-output syl))) (point))))
    (dtk-speak text)))

(defadvice its-delete-SYL (before emacspeak pre act)
  "Speak deleting char."
  (emacspeak-speak-this-char (char-after (1+ (point))) 'ja)
)

(defadvice its-hiragana (after emacspeak pre act)
  "Speak converted string"
  (emacspeak-egg-speak-after-conversion))

(defadvice its-katakana (after emacspeak pre act)
  "Speak converted string"
  (emacspeak-egg-speak-after-conversion))

(defadvice its-half-width (after emacspeak pre act)
  "Speak converted string"
  (emacspeak-egg-speak-after-conversion))

(defadvice its-full-width (after emacspeak pre act)
  "Speak converted string"
  (emacspeak-egg-speak-after-conversion))

(defun emacspeak-egg-speak-after-conversion ()
  "Read converted string"
  (let* ((converted-string
	  (if (egg-conversion-fence-p)
	      (egg-get-major-bunsetsu-converted
	       (egg-get-major-bunsetsu (point)))
	    (buffer-substring (its-search-beginning) (- (its-search-end) 1))))
	 (converted-string-phonetic
	  (emacspeak-ja-convert-string-to-phonetic converted-string))
	 )
    (dtk-speak converted-string-phonetic nil 'ja)
    ))
