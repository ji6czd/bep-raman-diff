;;;$Id: emacspeak-w3m.el,v 1.2 2004/11/28 09:13:40 inoue Exp $;;; emacspeak-w3m.el --- speech-enables w3m-el
;;{{{ Copyright
...
(defadvice w3m-mode (after emacspeak pre act comp)
  ...
  (setq w3m-after-cursor-move-hook
	'(w3m-highlight-current-anchor w3m-auto-show))
  (let ((w3m-binding (lookup-key w3m-mode-map emacspeak-prefix)))
    (setq emacspeak-w3m-keymap (make-keymap))
    (set-keymap-parent emacspeak-w3m-keymap emacspeak-keymap)
    (define-prefix-command 'emacspeak-w3m-prefix-command 'emacspeak-w3m-keymap)
    (define-key w3m-mode-map emacspeak-prefix 'emacspeak-w3m-prefix-command)
    (define-key emacspeak-w3m-keymap emacspeak-prefix w3m-binding))
  )
