...
;;; $Id: dtk-tcl.el,v 1.6 2002/02/06 15:40:29 inoue Exp $
;;; $Author: inoue $
...
(defsubst dtk-fix-control-chars ()
  ...
  (let ((char nil))
	...
	(or
	 (> char 255)  ; Avoid errors in multibyte is enabled
	 (replace-match
...
(defsubst  dtk-quote(mode )
  ...
  (let ((lang (or (get-text-property (point-min) 'emacspeak-language)
		  dtk-default-language))
	fix-characters-func)		; en is default
  (goto-char (point-min))
...
(defsubst dtk-set-language (language)
...
only speak upto the first ctrl-m."
  ...
  (unless  
	...
	(put-text-property 0 (length text)
			   ...
			   text))
      (when (not (get-text-property 0 'emacspeak-language text))
